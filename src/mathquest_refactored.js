(() => {
  ...
  const MONSTERS = [
    ...
    },
    {
      ...
      bg2: '#5a2248',
      sprites: {
        idle: spritePath('goblin_idle'),
        attack: spritePath('goblin_attack'),
        damage: spritePath('goblin_damage'),
        defeated: spritePath('goblin_defeated')
      }
    },
    {
  ...
  function updateUI() {
    ...
    const m = currentMonster();
    if (!els.monsterSprite.classList.contains('damage') && !els.monsterSprite.classList.contains('dead') && !els.monsterSprite.classList.contains('attack')) {
  ...
  function renderMonsterFrame(frame = 'idle') {
    ...
    const src = monster.sprites[frame] || monster.sprites.idle;
    const targets = [els.monsterSprite, els.menuMonsterSprite].filter(Boolean);
    targets.forEach(el => {
      el.innerHTML = `<img src="${src}" alt="${monster.name}: ${frame}" draggable="false">`;
    });
  }

  function setMonsterVisual() {
    els.monsterSprite.classList.remove('damage', 'dead', 'attack');
  ...
  function animateHeroAttack(crit) {
    ...
    setTimeout(() => {
      renderMonsterFrame('damage');
      els.monsterSprite.classList.add('damage');
      setTimeout(() => {
